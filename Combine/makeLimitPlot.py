...
from commonObjects import massBaseList, fwd__
...
def LimitVsM():
    ...
    for m in mass_interp:
        input_file = "{}/cards/higgsCombine_comb_{}.AsymptoticLimits.mH{}.root".format(fwd__, m, m)
...
def LimitSummary():
    ...
    cat_tag["untagged"]         = "Merged untagged combined"
    cat_tag["EE"]               = "Merged EE"
    cat_tag["EBLR9"]            = "Merged EB LR9"
    cat_tag["EBHR9"]            = "Merged EB HR9"
    cat_tag["tagged"]           = "Merged tagged combined"
    cat_tag["VBF"]              = "Merged VBF"
    cat_tag["BST"]              = "Merged Boost"
    cat_tag["Resolved"]         = "Resolved"
    
    # yhigh = [0, 2, 2.8, 3.6, 4.5, 6.5, 7.4, 8.3, 9.2, 11.2]
    yhigh = [0, 2.4, 3.4, 4.4, 5.4, 7.4, 8.4, 9.4, 11.4]
    ...
    for key, value in cat_tag.items():
        input_file = "{}/cards/higgsCombine_{}_125.AsymptoticLimits.mH125.root".format(fwd__, key)
        _limit = GetValuesFromFile(input_file)
        Limit["2sigma_do"].append(_limit[0])
        Limit["1sigma_do"].append(_limit[1])
        Limit["mean"].append(_limit[2])
...
def SigVsM():
    ...
    significance_inj = []
    for m in mass_interp:
        input_file = "{}/cards/higgsCombine_comb_{}_expSignal1.Significance.mH{}.root".format(fwd__, m, m)
        input_file_inj = "{}/cards/higgsCombine_comb_{}_expectSignalMass125.Significance.mH{}.root".format(fwd__, m, m)
    ...
    gL.GetYaxis().SetRangeUser(5*1e-4, 4)
    gL.GetYaxis().SetTickSize(0.03)
    ...
    gL.GetXaxis().SetLabelSize(0.04)
    gL.GetXaxis().SetTitleOffset(1.1)
    ...
    leg = ROOT.TLegend(0.2, 0.27, 0.77, 0.42)
    leg.SetTextSize(0.04)
    leg.SetFillStyle(0)
    leg.SetBorderSize(0)
    leg.SetFillColor(0)
    ...
    CMS_lumi(c1, 5, 10, "138 fb^{-1}", 2017, True, "Preliminary", "H #rightarrow #gamma*#gamma #rightarrow ee#gamma", "")
    c1.RedrawAxis()
    c1.Update()

...
std::vector<float> effSigma(TH1F* hist, double quantile = TMath::Erf(1.0/sqrt(2.0))){    
    std::vector<float> width_vec(3, 0.);
    ...
    if (total < 0){
        cout << Form(" --> integral of histogram is < 0 in effSigma calc: %s. Returning 0 for effSigma", hist->GetName()) << endl;
        return width_vec;
    }
    ...
    for(int iscan = -nrms; iscan < nrms+1; iscan++){ // Scan window centre
        ...
        for(int j = 1; j < nb; j++){
            if(jbm < nb) {
                ...
                cout << Form(" --> Reach bin = nBins in effSigma calc: %s. Returning 0. for effSigma", hist->GetName()) << endl;
                return width_vec;
            }
            ...
            if(kbm > 0) {
                ...
                cout << Form(" --> Reach bin = 0 in effSigma calc: %s. Returning 0. for effSigma", hist->GetName()) << endl;
                return width_vec;
            }
        }
